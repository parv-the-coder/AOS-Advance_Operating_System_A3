    ...
    unordered_map<string, function<void()>> cmdMap;
        ...
        cmdMap["download_file"] = [&]() {
            ...
            logincheck([&]() {
                ...
                // Helper: robustly read all bytes from a socket
                ...
                for (long long i = 0; i < num_pieces; ++i) s >> piece_hashes[i];
                ...
                queue<long long> pending_pieces;
                for (long long i = 0; i < num_pieces; ++i) if (piece_status[i] != 2) pending_pieces.push(i);
                mutex queue_mtx;
                const int MAX_RETRIES = 5;
                mutex state_mtx;
                atomic<long long> completed_count(0);
                    ...
                    while (true) {
                        ...
                        while (attempt < MAX_RETRIES && !success) {
                            for (int peer_idx : peer_order) {
                                ...
                                uint32_t piece_size;
                                if (!read_all(psock, (char*)&piece_size, sizeof(piece_size))) { close(psock); continue; }
                                piece_size = ntohl(piece_size);
                                if (piece_size > PIECE_SIZE) { close(psock); continue; }

                                vector<char> buffer(piece_size);
                                if (!read_all(psock, buffer.data(), piece_size)) { close(psock); continue; }
                                close(psock);

                                EVP_MD_CTX *ctx = EVP_MD_CTX_new();
                                EVP_DigestInit_ex(ctx, EVP_sha1(), NULL);
                                EVP_DigestUpdate(ctx, buffer.data(), piece_size);
